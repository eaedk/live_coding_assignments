...
from src.assign_003.script import solution
import pandas as pd
import os
...
def test_solution_True():
    ...
    valid_inputs = [
        {"filepath":"src/assets/geo_location_data.csv"},
    ]
    
    expected_df = [
        pd.read_csv(os.path.join(FILE_DIR, "data_closest.csv")).sort_values(by=["email"]).reset_index(drop=True),
    ]
...
def test_solution_False():
    ...
    valid_inputs = [
        {"filepath":"src/assets/processed_data.csv"},
    ]
